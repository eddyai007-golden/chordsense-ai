...
def resolve_rootless_voicings(chord_label, bass_idx, metadata, names):
    ...
    if bass_idx != root_idx:
        ...
        if interval == 4 and suffix == "m7":
            return f"{bass_name}maj9"

        if interval == 4 and suffix == "m7b5":
            return f"{bass_name}9"

        if interval == 4 and suffix == "dim7":
            return f"{bass_name}7b9"

        if interval == 10 and suffix == "maj7":
            return f"{bass_name}11"

        if interval == 7 and suffix == "m7":
            return f"{bass_name}11"
...
def analyze_chords(audio_path, start_time=0, end_time=None):
    ...
    times = librosa.frames_to_time(
        np.arange(len(smoothed_matches)),
        sr=sr,
        hop_length=512
    )
    ...
    for i in range(len(smoothed_matches)):
        ...
        final_chord = resolve_rootless_voicings(
            base_chord,
            bass_idx,
            metadata,
            names
